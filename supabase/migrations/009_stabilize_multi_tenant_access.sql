...
as $$
  select coalesce(
    (select up.institution_id from public.user_profiles up where up.id = auth.uid() limit 1),
    (select tp.institution_id from public.trainee_profiles tp where tp.user_id = auth.uid() limit 1),
    (select pp.institution_id from public.preceptor_profiles pp where pp.user_id = auth.uid() limit 1),
    (select ur.institution_id from public.user_roles ur where ur.user_id = auth.uid() and ur.institution_id is not null limit 1)
  );
$$;
...
as $$
  select (
    public.has_role('trainee')
    or public.has_role('trainee_me1')
    or public.has_role('trainee_me2')
    or public.has_role('trainee_me3')
  );
$$;
...
begin
  ...
  into existing_full_name, existing_institution_id
  ...
  if institution_id_value is null and role_code is not null and role_code <> 'super_admin' then
    select count(*), min(id)
    into institution_count, fallback_institution_id
